...
ssh $VULTR_USER@$VULTR_HOST << 'BACKEND_DEPLOY'
    ...
    sudo systemctl restart verzek-api.service 2>/dev/null || sudo systemctl restart backend-api 2>/dev/null || { echo "❌ Service restart failed"; exit 1; }
    
    echo "  → Waiting for service to stabilize..."
    sleep 5
    
    echo "  → Checking service status..."
    if sudo systemctl is-active --quiet verzek-api.service 2>/dev/null || sudo systemctl is-active --quiet backend-api 2>/dev/null; then
...
if curl -s -m 10 http://$VULTR_HOST:$BACKEND_PORT/api/ping 2>/dev/null | grep -q "ok\|pong\|OK"; then
    echo "  ✅ Backend API responding on port $BACKEND_PORT"
else
    echo "  ⚠️  Backend API check inconclusive (may need manual verification on port $BACKEND_PORT)"
fi

echo "  → Checking for recent errors in backend logs..."
ssh $VULTR_USER@$VULTR_HOST "journalctl -u verzek-api.service -n 30 --no-pager --since '5 minutes ago' 2>/dev/null | grep -i error || journalctl -u backend-api -n 30 --no-pager --since '5 minutes ago' 2>/dev/null | grep -i error || echo '  ℹ️  No recent errors found'"
...
ssh $VULTR_USER@$VULTR_HOST << 'STATUS'
    BACKEND_STATUS=$(sudo systemctl is-active verzek-api.service 2>/dev/null || sudo systemctl is-active backend-api 2>/dev/null || echo "unknown")
    ENGINE_STATUS=$(sudo systemctl is-active verzek-signalengine.service 2>/dev/null || sudo systemctl is-active signal-engine 2>/dev/null || echo "unknown")
...
echo ""
echo "🔗 Next Steps:"
echo "  1. Test backend: curl http://$VULTR_HOST:$BACKEND_PORT/api/ping"
