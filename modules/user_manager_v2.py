...
class UserManager:
    """Manages all users and their settings - SQLite backend"""
    ...
    def _load_users_from_db(self):
        ...
        try:
            ...
            for db_user in db_users:
                if db_user['data']:
                    user = User.from_dict(db_user['data'])
    ...
    def _save_user_to_db(self, user: User):
        ...
        try:
            ...
            existing = self.db.get_user(user.user_id)
            if existing:
                self.db.update_user(
                    user.user_id,
                    ...
                    subscription_expires=user.plan_expires_at,
    ...
    def get_user_by_email(self, email: str) -> Optional[User]:
        ...
        db_user = self.db.get_user_by_email(email_lower)
        if db_user and db_user['data']:
            user = User.from_dict(db_user['data'])
            self.users[user.user_id] = user
            return user
